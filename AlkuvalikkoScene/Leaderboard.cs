...
public class Leaderboard : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        apiKeyCode = EnvManager.Instance.Get("apiKeyCode"); //json.env containing keys and values in Assets/Resources
        endpoint = EnvManager.Instance.Get("ENDPOINT_URL");

        InfoText.gameObject.SetActive(true);
        InfoText.text = "Ladataan pistetietoja";
    ...
    IEnumerator PostData(string dataStr, int time) {
        //const string apiKeyCode = "master_salis";
        string json = "{\"username\":\"" + dataStr + "\", \"time\":\"" + time + "\", \"keyCode\":\"" + apiKeyCode + "\"}";
    ...
    IEnumerator PutData(string dataStr, int time, string id) {
        ...
        UnityWebRequest www = new UnityWebRequest(endpoint + "/" + id, "PUT");   
        www.uploadHandler = new UploadHandlerRaw(jsonBytes);
        www.downloadHandler = new DownloadHandlerBuffer();
        www.SetRequestHeader("Content-Type", "application/json");
...
public void Submit() {
        ...
        if (PlayerPrefs.HasKey("BestTimeLevel1") && PlayerPrefs.HasKey("BestTimeLevel2") && PlayerPrefs.HasKey("BestTimeLevel3"))
        {
            ...
            else
            {
                ...
                InfoText.gameObject.SetActive(true);
                InfoText.text = "Pelaajanimi ei voi olla tyhjä";
            }
        }
        else
        {
            ...
            InfoText.gameObject.SetActive(true);
            InfoText.text = "Jokainen taso tulee läpäistä ennen tuloksen lähetämistä";
        }
}
